...
/**
 * Current pseudonymisation primitive — HMAC-SHA-256 keyed by ENCRYPTION_KEY.
 * Use this for all new IP/UA hashes written to audit logs.
 */
export const hmacSha256Hex = async (
	input: string,
	env: Env,
): Promise<string> => {
	const keyBytes = env.ENCRYPTION_KEY ? base64ToBytes(env.ENCRYPTION_KEY) : new Uint8Array(0);
